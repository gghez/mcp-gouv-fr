...
async def test_build_server_has_tools() -> None:
    ...
    assert "insee_search_unites_legales" in names
    assert "insee_search_etablissements" in names
...
async def test_tools_expose_pydantic_output_schema() -> None:
    ...
    insee_search_ul = tools["insee_search_unites_legales"]
    ...
    assert insee_search_ul.output_schema is not None
