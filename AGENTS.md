...
- **Composite server**: `mcp_gouv_fr.server.build_server()` only mounts API packages; it has no tools of its own. Each package under `apis/<name>/` exposes `build_subserver()` with its own **lifespan** (e.g. a dedicated `httpx.AsyncClient` in `apis/<name>/server.py`). The lifespan stores the client directly on the server instance (`server._http_client = client`) and tools retrieve it with `get_http_client(ctx)` from `mcp_gouv_fr.http_lifespan`. This sidesteps FastMCP's broken lifespan-context propagation for mounted sub-servers (`FastMCPProvider.lifespan()` discards the yielded dict). API-specific extras (e.g. Radio France `_graphql_url`) are stored the same way.
